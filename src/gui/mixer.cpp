...
bool FurnaceGUI::portSet(String label, unsigned int portSetID, int ins, int outs, int activeIns, int activeOuts, int& clickedPort, std::map<unsigned int,ImVec2>& portPos) {
  ...
  // input ports
  for (int i=0; i<ins; i++) {
    String portLabel=_L("input##sgmx");
    String subPortID=fmt::sprintf("subPort%.5x",(portSetID<<4)|i);
    if (ins==2) {
      portLabel=_L(portNamesStereo[i&1]);
    } else if (ins>2) {
  ...
  for (int i=0; i<outs; i++) {
    String portLabel=_L("output##sgmx");
    String subPortID=fmt::sprintf("subPort%.5x",(portSetID<<4)|i);
    if (outs==2) {
      portLabel=_L(portNamesStereo[i&1]);
    } else if (outs>2) {
...
void FurnaceGUI::drawMixer() {
  ...
  if (ImGui::Begin("Mixer",&mixerOpen,globalWinFlags|(settings.allowEditDocking?0:ImGuiWindowFlags_NoDocking), _L("Mixer###Mixer"))) {
    if (ImGui::BeginTabBar("MixerView")) {
      if (ImGui::BeginTabItem(_L("Mixer##sgmx"))) {
        if (ImGui::SliderFloat(_L("Master Volume##sgmx"),&e->song.masterVol,0,3,"%.2fx")) {
        ...
        if (ImGui::BeginTable("CurPortSet",2)) {
          ...
          for (int i=0; i<e->song.systemLen; i++) {
            ...
            ImGui::TableNextColumn();
            if (ImGui::Checkbox(_L("Invert##sgmx"),&doInvert)) {
      ...
      if (ImGui::BeginTabItem(_L("Patchbay##sgmx"))) {
        std::map<unsigned int,ImVec2> portPos;

        if (ImGui::BeginTable("PatchbayOptions",3)) {
          ImGui::TableNextRow();
          ImGui::TableNextColumn();
          if (ImGui::Checkbox(_L("Automatic patchbay##sgmx"),&e->song.patchbayAuto)) {
            if (e->song.patchbayAuto) e->autoPatchbayP();
            MARK_MODIFIED;
          }
          ImGui::TableNextColumn();
          ImGui::Checkbox(_L("Display hidden ports##sgmx"),&displayHiddenPorts);
          ImGui::TableNextColumn();
          ImGui::Checkbox(_L("Display internal##sgmx"),&displayInternalPorts);
          ImGui::EndTable();
        }
        ...
        if (ImGui::BeginChild("Patchbay",ImVec2(0,0),true)) {
          ...
          ImVec2 sysSize=calcPortSetSize(_L("System##sgmx0"),displayHiddenPorts?DIV_MAX_OUTPUTS:e->getAudioDescGot().outChans,0);
          ...
          if (displayInternalPorts) {
            if (portSet(_L("Sample Preview##sgmx"),0xffd,0,1,0,1,selectedSubPort,portPos)) {
            ...
            if (portSet(_L("Metronome##sgmx"),0xffe,0,1,0,1,selectedSubPort,portPos)) {
          ...
          if (portSet(_L("System##sgmx1"),0x1000,displayHiddenPorts?DIV_MAX_OUTPUTS:e->getAudioDescGot().outChans,0,e->getAudioDescGot().outChans,0,selectedSubPort,portPos)) {
        ...
        if (ImGui::BeginPopup("SubPortOptions",ImGuiWindowFlags_NoMove|ImGuiWindowFlags_AlwaysAutoResize|ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoSavedSettings)) {
          if (ImGui::MenuItem(_L("disconnect all##sgmx"))) {
