...
void FurnaceGUI::drawOsc() {
  ...
  if (ImGui::Begin("Oscilloscope",&oscOpen,globalWinFlags,_L("Oscilloscope###Oscilloscope"))) {
    if (oscZoomSlider) {
      ...
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(_L("zoom: %.2fx (%.1fdB)##sgos"),oscZoom,20.0*log10(oscZoom*2.0));
      }
      ...
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(_L("window size: %.1fms##sgos"),oscWindowSize);
      }
      if (ImGui::IsItemClicked(ImGuiMouseButton_Middle)) {
        oscWindowSize=20.0;
      }
    ...
    if (oscZoomSlider && ImGui::IsItemHovered()) {
      ...
      if (val<=-INFINITY) {
        ImGui::SetTooltip(_L("(-Infinity)dB##sgos"));
      } else {
