...
void FurnaceGUI::drawActualInsEditor()
{
  ...
  if (ImGui::BeginTable("InsProp",3)) {
    ...
    if (ImGui::BeginPopupContextItem("InsSaveFormats",ImGuiMouseButton_Right)) {
      if (ImGui::MenuItem(_("export .dmp..."))) {
    ...
    if (ImGui::BeginCombo("##Type",(insType>=DIV_INS_MAX)?_("Unknown"):_L(insTypes[insType][0]))) {
...
void FurnaceGUI::drawInsEdit() {
  ...
  if (ImGui::Begin("Instrument Editor",&insEditOpen,globalWinFlags|(settings.allowEditDocking?0:ImGuiWindowFlags_NoDocking), _("Instrument Editor###Instrument Editor"))) {
    if (curIns==-2) {
      ...
      CENTER_TEXT(_("no instrument selected##sgie0"));
      ...
      if (ImGui::BeginTable("noAssetCenter",3)) {
        ...
        if (e->song.ins.size()>0) {
          ...
          ImGui::TextUnformatted(_("or##sgie0"));
          ImGui::SameLine();
        }
