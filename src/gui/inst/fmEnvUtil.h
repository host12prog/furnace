...
#define FM_NAME(x) (settings.language == 0 ? EngFmParamNames[settings.fmNames][x] : _L(fmParamNames[settings.fmNames][x]))
#define FM_SHORT_NAME(x) _L(fmParamShortNames[settings.fmNames][x])
#define ESFM_LONG_NAME(x) (settings.language == 0 ? EngEsfmParamLongNames[x] : _L(esfmParamLongNames[x]))
...
#define OP_DRAG_POINT \
  ...
  if (ImGui::BeginDragDropSource()) { \
    ...
    if (ImGui::IsKeyDown(ImGuiKey_LeftShift) || ImGui::IsKeyDown(ImGuiKey_RightShift)) { \
      ImGui::Text(_("(copying)##sgifmeh")); \
    } else { \
      ImGui::Text(_("(swapping)##sgifmeh")); \
    } \
