...
void FurnaceGUI::drawInsAY(DivInstrument* ins)
{
  ...
  if (ImGui::BeginTabItem(_L("Macros##sgiAY"))) 
  {
    ...
    macroList.push_back(FurnaceGUIMacroDesc(_L("Arpeggio##sgiAY"),ins,DIV_MACRO_ARP,0xff,-120,120,160,uiColors[GUI_COLOR_MACRO_PITCH],true,NULL,macroHoverNote,false,NULL,0,true,ins->std.get_macro(DIV_MACRO_ARP, true)->val));
    ...
    if(!ins->amiga.useSample)
    {
      ...
      macroList.push_back(FurnaceGUIMacroDesc(_L("Waveform##sgiAY"),ins,DIV_MACRO_WAVE,0xff,0,3,64,uiColors[GUI_COLOR_MACRO_WAVE],false,NULL,NULL,true,ayShapeBits,1));
    }
