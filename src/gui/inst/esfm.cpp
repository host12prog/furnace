...
void FurnaceGUI::drawInsESFM(DivInstrument* ins)
{
  ...
  if (ImGui::BeginTabItem("FM")) 
  {
    ...
    if (settings.fmLayout==0) 
    {
      ...
      if (ImGui::BeginTable("FMOperators",numCols,ImGuiTableFlags_SizingStretchProp|ImGuiTableFlags_BordersH|ImGuiTableFlags_BordersOuterV)) 
      {
        ...
        ImGui::TableNextColumn();
        CENTER_TEXT(_("Other##sgiESFM0"));
        ImGui::TextUnformatted(_("Other##sgiESFM1"));
        ImGui::TableNextColumn();
        ImGui::TableNextColumn();
        CENTER_TEXT(FM_NAME(FM_WS));
        ImGui::TextUnformatted(FM_NAME(FM_WS));
        ImGui::TableNextColumn();
        CENTER_TEXT(_("Envelope##sgiESFM0"));
        ImGui::TextUnformatted(_("Envelope##sgiESFM1"));
        ...
        for (int i=0; i<opCount; i++) 
        {
          ...
          ImGui::PushID(fmt::sprintf(_("op%d##sgiESFM0"),i).c_str());
          String opNameLabel;
          opNameLabel=fmt::sprintf(_("OP%d##sgiESFM1"),i+1);
          ...
          {
            ImGui::SetTooltip(_("Detune in semitones##sgiESFM0"));
          }
          ...
          if (ImGui::IsItemHovered()) 
          {
            ImGui::SetTooltip(_("Detune in fractions of semitone.\n128 = +1 semitone, -128 = -1 semitone.##sgiESFM0"));
          }
          ...
          if (ImGui::BeginTable("panCheckboxes",(fixedOn)?3:2,ImGuiTableFlags_SizingStretchProp)) 
          {
            ...
            if (ImGui::IsItemHovered()) 
            {
              ImGui::SetTooltip(_("If operator outputs sound, enable left channel output.##sgiESFM0"));
            }
            ImGui::TableNextColumn();
            ImGui::SetCursorPosY(yCoordBeforeTablePadding);
            ...
            if (ImGui::IsItemHovered()) 
            {
              ImGui::SetTooltip(_("If operator outputs sound, enable right channel output.##sgiESFM0"));
            }
            if (fixedOn) 
            {
              ImGui::TableNextColumn();
          ...
          if (fixedOn) 
          {
            ...
            if (ImGui::InputInt(_("Block##sgiESFM0"),&block,1,1)) 
            {
              if (block<0) block=0;
              if (block>7) block=7;
              opE.ct=(opE.ct&(~(7<<2)))|(block<<2);
            }
            if (ImGui::InputInt(_("FreqNum##sgiESFM0"),&freqNum,1,16)) 
            {
    ...
    { // alternate
      ...
      if (ImGui::BeginTable("AltFMOperators",columns,ImGuiTableFlags_SizingStretchSame|ImGuiTableFlags_BordersInner)) 
      {
        for (int i=0; i<opCount; i++) 
        {
          ...
          ImGui::TableNextColumn();
          ImGui::PushID(fmt::sprintf(_("op%d##sgiESFM2"),i).c_str());
          ...
          snprintf(tempID,1024,_("Operator %d##sgiESFM"),i+1);
          ...
          if (ImGui::BeginTable("opParams",4,ImGuiTableFlags_BordersInnerV)) 
          {
            ...
            TOOLTIP_TEXT(ESFM_LONG_NAME(ESFM_DELAY));
            ImGui::TableNextColumn();
            ImGui::Text(_("Waveform##sgiESFM"));
            ImGui::TableNextColumn();
            ImGui::Text(_("Envelope##sgiESFM"));
            ImGui::TableNextColumn();
            ...
            if (opE.fixed) 
            {
              int block=(opE.ct>>2)&7;
              int freqNum=((opE.ct&3)<<8)|((unsigned char)opE.dt);
              ImGui::Text(_("Blk##sgiESFM"));
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("Block##sgiESFM1"));
              }
              ImGui::SameLine();
              ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
              ...
              ImGui::Text(_("F##sgiESFM"));
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("Frequency (F-Num)##sgiESFM"));
              }
              ImGui::SameLine();
            ...
            else 
            {
              ...
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("Detune in semitones##sgiESFM1"));
              }
            ...
            if (ImGui::BeginTable("panCheckboxes",2,ImGuiTableFlags_SizingStretchSame)) 
            {
              ...
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("If operator outputs sound, enable left channel output.##sgiESFM1"));
              }
              ImGui::TableNextColumn();
              ImGui::SetCursorPosY(yPosOutsideTablePadding);
              if (ImGui::Checkbox(ESFM_SHORT_NAME(ESFM_RIGHT),&rightOn)) 
              ...
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("If operator outputs sound, enable right channel output.##sgiESFM1"));
              }
              ImGui::EndTable();
            }
    ...
    { // classic
      ...
      if (ImGui::BeginTable("FMOperators",columns,ImGuiTableFlags_SizingStretchSame)) 
      {
        for (int i=0; i<opCount; i++) {
          ...
          ImGui::TableNextColumn();
          ImGui::Separator();
          ImGui::PushID(fmt::sprintf(_("op%d##sgiESFM3"),i).c_str());
          ...
          OP_DRAG_POINT;
          ImGui::SameLine();
          opNameLabel=fmt::sprintf(_("OP%d##sgiESFM4"),i+1);
          ...
          if (ImGui::BeginTable("opParams",2,ImGuiTableFlags_SizingStretchProp)) 
          {
            ...
            else 
            {
              ...
              if (ImGui::IsItemHovered()) 
              {
                ImGui::SetTooltip(_("Detune in semitones##sgiESFM2"));
              }
              ImGui::TableNextColumn();
          ...
          if (ImGui::IsItemHovered()) 
          {
            ImGui::SetTooltip(_("If operator outputs sound, enable left channel output.##sgiESFM2"));
          }
          ImGui::SameLine();
          ...
          if (ImGui::IsItemHovered()) 
          {
            ImGui::SetTooltip(_("If operator outputs sound, enable right channel output.##sgiESFM2"));
          }
          ImGui::SameLine();
          if (ImGui::Checkbox(ESFM_NAME(ESFM_FIXED),&fixedOn)) 
          { PARAMETER
  ...
  for (int i=0; i<opCount; i++) 
  {
    snprintf(label,31,_("OP%d Macros##sgiESFM"),i+1);
    ...
    if (ImGui::BeginTabItem(label)) 
    {
      ...
      {
        macroList.push_back(FurnaceGUIMacroDesc(_("Block##sgiESFM3"),ins,(DivMacroType)DIV_MACRO_OP_SSG,ordi,0,7,64,uiColors[GUI_COLOR_MACRO_OTHER],true));
        macroList.push_back(FurnaceGUIMacroDesc(_("FreqNum##sgiESFM2"),ins,(DivMacroType)DIV_MACRO_OP_DT,ordi,0,1023,160,uiColors[GUI_COLOR_MACRO_OTHER]));
      }

      else 
      {
        macroList.push_back(FurnaceGUIMacroDesc(_("Op. Arpeggio##sgiESFM"),ins,(DivMacroType)DIV_MACRO_OP_SSG,ordi,-120,120,160,uiColors[GUI_COLOR_MACRO_PITCH],true,NULL,macroHoverNote,false,NULL,true,ins->std.ops[ordi].op_get_macro(DIV_MACRO_OP_SSG, true)->val,true));
        macroList.push_back(FurnaceGUIMacroDesc(_("Op. Pitch##sgiESFM"),ins,(DivMacroType)DIV_MACRO_OP_DT,ordi,-2048,2047,160,uiColors[GUI_COLOR_MACRO_PITCH],true,macroRelativeMode,NULL,false,NULL,false,NULL,false,true));
      }
      ...
      macroList.push_back(FurnaceGUIMacroDesc(_("Op. Panning##sgiESFM"),ins,(DivMacroType)DIV_MACRO_OP_RS,ordi,0,2,32,uiColors[GUI_COLOR_MACRO_OTHER],false,NULL,NULL,true,panBits));
  ...
  if (ImGui::BeginTabItem(_("Macros##sgiESFM"))) 
  {
    panMax=2;

    macroList.push_back(FurnaceGUIMacroDesc(_("Volume##sgiESFM"),ins,DIV_MACRO_VOL,0xff,0,63,160,uiColors[GUI_COLOR_MACRO_VOLUME]));
    macroList.push_back(FurnaceGUIMacroDesc(_("Arpeggio##sgiESFM"),ins,DIV_MACRO_ARP,0xff,-120,120,160,uiColors[GUI_COLOR_MACRO_PITCH],true,NULL,macroHoverNote,false,NULL,true,ins->std.get_macro(DIV_MACRO_ARP, true)->val));
    macroList.push_back(FurnaceGUIMacroDesc(_("Pitch##sgiESFM"),ins,DIV_MACRO_PITCH,0xff,-2048,2047,160,uiColors[GUI_COLOR_MACRO_PITCH],true,macroRelativeMode));
    macroList.push_back(FurnaceGUIMacroDesc(_("OP4 Noise Mode##sgiESFM"),ins,DIV_MACRO_DUTY,0xff,0,3,32,uiColors[GUI_COLOR_MACRO_OTHER]));
    macroList.push_back(FurnaceGUIMacroDesc(_("Panning##sgiESFM"),ins,DIV_MACRO_PAN_LEFT,0xff,0,panMax,32,uiColors[GUI_COLOR_MACRO_OTHER],false,NULL,NULL,true,panBits));

    macroList.push_back(FurnaceGUIMacroDesc(_("Phase Reset##sgiESFM"),ins,DIV_MACRO_PHASE_RESET,0xff,0,1,32,uiColors[GUI_COLOR_MACRO_OTHER],false,NULL,NULL,true));
