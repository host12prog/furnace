...
void DivPlatformSAA1099::tick(bool sysTick) {
  for (int i=0; i<6; i++) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_DUTY)->had) {
      saaNoise[i/3]=chan[i].std.get_div_macro_struct(DIV_MACRO_DUTY)->val&3;
      rWrite(0x16,saaNoise[0]|(saaNoise[1]<<4));
    }
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->had) {
      chan[i].psgMode=chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->val&3;
    }
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_EX1)->had) {
      saaEnv[i/3]=chan[i].std.get_div_macro_struct(DIV_MACRO_EX1)->val;
