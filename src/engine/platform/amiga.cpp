...
void DivPlatformAmiga::tick(bool sysTick) {
  for (int i=0; i<4; i++) {
    ...
    if (NEW_ARP_STRAT) {
      ...
      chan[i].baseFreq=round(NOTE_PERIODIC_NOROUND(parent->calcArp(chan[i].note,chan[i].std.get_div_macro_struct(DIV_MACRO_ARP)->val)));
...
int DivPlatformAmiga::dispatch(DivCommand c) {
  switch (c.cmd) {
    ...
    case DIV_CMD_LEGATO: {
      chan[c.chan].baseFreq=round(NOTE_PERIODIC_NOROUND(c.value+chan[c.chan].sampleNoteDelta+((HACKY_LEGATO_MESS)?(chan[c.chan].std.get_div_macro_struct(DIV_MACRO_ARP)->val):(0))));
      chan[c.chan].freqChanged=true;
      chan[c.chan].note=c.value;
      break;
    }
