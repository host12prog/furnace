...
void DivPlatformLynx::tick(bool sysTick) {
  for (int i=0; i<4; i++) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_EX1)->had) {
      WRITE_LFSR(i, chan[i].std.get_div_macro_struct(DIV_MACRO_EX1)->val&0xff);
      WRITE_OTHER(i, (chan[i].std.get_div_macro_struct(DIV_MACRO_EX1)->val&0xf00)>>4);
    }
...
int DivPlatformLynx::dispatch(DivCommand c) {
  ...
  switch (c.cmd) {
    ...
    case DIV_CMD_LEGATO: {
      int whatAMess=c.value+chan[c.chan].sampleNoteDelta+((HACKY_LEGATO_MESS)?(chan[c.chan].std.get_div_macro_struct(DIV_MACRO_ARP)->val):(0));
