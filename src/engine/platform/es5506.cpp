...
void DivPlatformES5506::tick(bool sysTick) {
  for (int i=0; i<=chanMax; i++) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_VOL)->had) {
      int nextVol;
      if (amigaVol) {
        nextVol=VOL_SCALE_LINEAR(MIN(64,chan[i].vol),MIN(64,chan[i].std.get_div_macro_struct(DIV_MACRO_VOL)->val),64);
      } else {
        nextVol=VOL_SCALE_LOG((0xfff*chan[i].vol)/0xff,(0xfff*chan[i].std.get_div_macro_struct(DIV_MACRO_VOL)->val)/chan[i].volMacroMax,0xfff);
      }
...
int DivPlatformES5506::dispatch(DivCommand c) {
  switch (c.cmd) {
    ...
    case DIV_CMD_LEGATO: {
      chan[c.chan].note=c.value;
      chan[c.chan].nextNote=chan[c.chan].note+chan[c.chan].sampleNoteDelta+((HACKY_LEGATO_MESS)?(chan[c.chan].std.get_div_macro_struct(DIV_MACRO_ARP)->val-12):(0));
