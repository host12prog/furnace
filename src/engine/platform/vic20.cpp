...
void DivPlatformVIC20::tick(bool sysTick) {
  for (int i=0; i<4; i++) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_DUTY)->had) {
      if (chan[i].onOff!=(bool)chan[i].std.get_div_macro_struct(DIV_MACRO_DUTY)->val) {
        chan[i].onOff=(bool)chan[i].std.get_div_macro_struct(DIV_MACRO_DUTY)->val;
        if (chan[i].active) {
          if (chan[i].onOff) {
            chan[i].keyOn=true;
            chan[i].keyOff=false;
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->had) {
      if (chan[i].wave!=chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->val) {
        chan[i].wave=chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->val&0x0f;
        chan[i].keyOn=true;
      }
    }
