  ...
  for (int i=0; i<3; i++) {
    ...
    */
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_VOL)->had) {
      chan[i].outVol=((chan[i].vol&31)*MIN(31,chan[i].std.get_div_macro_struct(DIV_MACRO_VOL)->val))>>5;
      if (!isMuted[i]) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->had) {
      if (chan[i].wave!=chan[i].std.get_div_macro_struct(DIV_MACRO_WAVE)->val || chan[i].ws.activeChanged()) {
        ...
        chan[i].ws.changeWave1(chan[i].wave & 0xff, false, (chan[i].wave & (1 << 30)) ? true : false, parent->getIns(chan[i].ins,DIV_INS_SM8521));
    ...
    if (chan[i].active) {
      if (chan[i].ws.tick() || (chan[i].std.get_div_macro_struct(DIV_MACRO_PHASE_RESET)->had && chan[i].std.get_div_macro_struct(DIV_MACRO_PHASE_RESET)->val==1)) {
        updateWave(i);
      }
    }
  ...
  switch (c.cmd) {
    case DIV_CMD_NOTE_ON: {
      ...
      if (chan[c.chan].wave<0) {
        chan[c.chan].wave=0;
        chan[c.chan].ws.changeWave1(chan[c.chan].wave & 0xff, false, (chan[c.chan].wave & (1 << 30)) ? true : false, parent->getIns(chan[c.chan].ins,DIV_INS_SM8521));
      }
...
void DivPlatformSM8521::notifyWaveChange(int wave) {
  for (int i=0; i<2; i++) {
    if (chan[i].wave==wave) {
      chan[i].ws.changeWave1(wave & 0xff, false, (wave & (1 << 30)) ? true : false, parent->getIns(chan[i].ins,DIV_INS_SM8521));
      updateWave(i);
    }
  }
}
