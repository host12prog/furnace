...
void readEnvelope(DivInstrument* ins, int env, unsigned char flags, unsigned char numPoints, unsigned char loopStart, unsigned char loopEnd, unsigned char susPoint, short* points) {
  ...
  if (env==1) {
    for (int i=0; i<ins->std.get_macro(DIV_MACRO_PAN_LEFT, true)->len; i++) {
      int val=ins->std.get_macro(DIV_MACRO_PAN_LEFT, true)->val[i]-32;
      if (val==0) {
    ...
    for (int i=0; i<ds.insLen; i++) {
      ...
      if (sampleCount>0) {
        ...
        for (int j=0; j<96; j++) {
          ...
          ins->amiga.get_amiga_sample_map(j, true)->map=ds.sample.size()+nextMap;
          noteMap[i][j]=nextMap;
        }
        ...
        if (volType&1) {
          ...
          if (volFade!=0) {
            int cur=64;
            int macroLen=ins->std.get_macro(DIV_MACRO_VOL, true)->len;
            int curPos=ins->std.get_macro(DIV_MACRO_VOL, true)->len-1;
            if (ins->std.get_macro(DIV_MACRO_VOL, true)->loop<macroLen) {
              curPos=ins->std.get_macro(DIV_MACRO_VOL, true)->loop;
            }
            if (ins->std.get_macro(DIV_MACRO_VOL, true)->len>0) {
              cur=ins->std.get_macro(DIV_MACRO_VOL, true)->val[curPos];
            }
            ...
            if (ins->std.get_macro(DIV_MACRO_VOL, true)->rel<ins->std.get_macro(DIV_MACRO_VOL, true)->len && ins->std.get_macro(DIV_MACRO_VOL, true)->rel<ins->std.get_macro(DIV_MACRO_VOL, true)->loop) {
              ins->std.get_macro(DIV_MACRO_VOL, true)->loop=255;
            }
          }
        } else {
