...
void DivLocale::addTranslationsPolish()
{
    ...
    strings["if this is on, 17xx has no effect on the operator channels in YM2612.##sgcf"].plurals[0] = "при включении этого флага 17xx не работает, если размещён на каналах operatorов расширенного канала (для YM2612).";
    ...
    strings["Ins change in ExtCh operator 2-4 affects FB (compatibility)##sgsc1"].plurals[0] = "Смена инструмента на operatoraх 2-4 расш. канала меняет FB (совместимость)";
    ...
    strings["Ins change in ExtCh operator 2-4 affects FB (compatibility)##sgsc2"].plurals[0] = "Смена инструмента на operatoraх 2-4 расш. канала меняет FB (совместимость)";
    ...
    strings["Ins change in ExtCh operator 2-4 affects FB (compatibility)##sgsc3"].plurals[0] = "Смена инструмента на operatoraх 2-4 расш. канала меняет FB (совместимость)";
    ...
    strings["Use separate colors for carriers/modulators in FM editor##sgse"].plurals[0] = "Использовать различные цвета для модулирующих и несущих operatorов";
    ...
    strings["Mod. accent (primary)##CC_GUI_COLOR_FM_PRIMARY_MOD"].plurals[0] = "Оттенок модулирующего operatora (основной)##CC_GUI_COLOR_FM_PRIMARY_MOD";
    strings["Mod. accent (secondary)##CC_GUI_COLOR_FM_SECONDARY_MOD"].plurals[0] = "Оттенок модулирующего operatora (неосновной)##CC_GUI_COLOR_FM_SECONDARY_MOD";
    strings["Mod. border##CC_GUI_COLOR_FM_BORDER_MOD"].plurals[0] = "Граница модулирующего operatora##CC_GUI_COLOR_FM_BORDER_MOD";
    strings["Mod. border shadow##CC_GUI_COLOR_FM_BORDER_SHADOW_MOD"].plurals[0] = "Тень границы модулирующего operatora##CC_GUI_COLOR_FM_BORDER_SHADOW_MOD";

    strings["Car. accent (primary)##CC_GUI_COLOR_FM_PRIMARY_CAR"].plurals[0] = "Оттенок несущего operatora (основной)##CC_GUI_COLOR_FM_PRIMARY_CAR";
    strings["Car. accent (secondary)##CC_GUI_COLOR_FM_SECONDARY_CAR"].plurals[0] = "Оттенок несущего operatora (неосновной)##CC_GUI_COLOR_FM_SECONDARY_CAR";
    strings["Car. border##CC_GUI_COLOR_FM_BORDER_CAR"].plurals[0] = "Граница несущего operatora##CC_GUI_COLOR_FM_BORDER_CAR";
    strings["Car. border shadow##CC_GUI_COLOR_FM_BORDER_SHADOW_CAR"].plurals[0] = "Тень границы несущего operatora##CC_GUI_COLOR_FM_BORDER_SHADOW_CAR";
    ...
    strings["07xy: Tremolo (x: speed; y: depth)##seen"].plurals[0] = "07xy: Tremolo (x: szybkość; y: głebokość)";
    ...
    strings["Cxxx: Set tick rate (hz)##seen"].plurals[0] = "Cxxx: Установить частоту движка трекера (Hz)";
    ...
    strings["16xy: Set operator multiplier (x: operator from 1 to 4; y: multiplier)##sesd0"].plurals[0] = "16xy: Множитель частоты (x: operator (1-4); y: множитель)";
    ...
    strings["50xy: Set AM (x: operator from 1 to 4 (0 for all ops); y: AM)##sesd0"].plurals[0] = "50xy: Включить АМ (x: operator 1-4 (0 dla wszystkich operatorów); y: АМ)";
    strings["51xy: Set sustain level (x: operator from 1 to 4 (0 for all ops); y: sustain)##sesd0"].plurals[0] = "51xy: Уровень сустейна (x: operator 1-4 (0 dla wszystkich operatorów); y: уровень)";
    strings["52xy: Set release (x: operator from 1 to 4 (0 for all ops); y: release)##sesd0"].plurals[0] = "52xy: Релиз (x: operator 1-4 (0 dla wszystkich operatorów); y: релиз)";
    strings["53xy: Set detune (x: operator from 1 to 4 (0 for all ops); y: detune where 3 is center)##sesd"].plurals[0] = "53xy: Расстройка (x: operator 1-4 (0 dla wszystkich operatorów); y: расстройка (3 - без расстройки))";
    strings["54xy: Set envelope scale (x: operator from 1 to 4 (0 for all ops); y: scale from 0 to 3)##sesd0"].plurals[0] = "54xy: Масш. огиб. (x: operator 1-4 (0 dla wszystkich operatorów); y: масш. 0-3)";
    ...
    strings["55xy: Set detune 2 (x: operator from 1 to 4 (0 for all ops); y: detune from 0 to 3)##sesd"].plurals[0] = "55xy: Расстройка 2 (x: operator 1-4 (0 dla wszystkich operatorów); y: расстройка 0-3)";
    ...
    strings["28xy: Set reverb (x: operator from 1 to 4 (0 for all ops); y: reverb from 0 to 7)##sesd"].plurals[0] = "28xy: Реверб (x: operator 1-4 (0 dla wszystkich operatorów); y: реверб 0-7)";
    strings["2Axy: Set waveform (x: operator from 1 to 4 (0 for all ops); y: waveform from 0 to 7)##sesd0"].plurals[0] = "2Axy: Форма волны (x: operator 1-4 (0 dla wszystkich operatorów); y: форма волны 0-7)";
    strings["2Bxy: Set envelope generator shift (x: operator from 1 to 4 (0 for all ops); y: shift from 0 to 3)##sesd"].plurals[0] = "2Bxy: Сдвиг генератора огибающей (x: operator 1-4 (0 dla wszystkich operatorów); y: сдвиг 0-3)";
    strings["2Cxy: Set fine multiplier (x: operator from 1 to 4 (0 for all ops); y: fine)##sesd"].plurals[0] = "2Cxy: Точный множитель (x: operator 1-4 (0 dla wszystkich operatorów); y: множитель)";
    strings["3xyy: Set fixed frequency of operator 1 (x: octave from 0 to 7; y: frequency)##sesd"].plurals[0] = "3xyy: Установить фиксированную частоту operatora 1 (x: октава 0-7; y: частота)";
    strings["3xyy: Set fixed frequency of operator 2 (x: octave from 8 to F; y: frequency)##sesd"].plurals[0] = "3xyy: Установить фиксированную частоту operatora 2 (x: октава 8-F; y: частота)";
    strings["4xyy: Set fixed frequency of operator 3 (x: octave from 0 to 7; y: frequency)##sesd"].plurals[0] = "4xyy: Установить фиксированную частоту operatora 3 (x: октава 0-7; y: частота)";
    strings["4xyy: Set fixed frequency of operator 4 (x: octave from 8 to F; y: frequency)##sesd"].plurals[0] = "4xyy: Установить фиксированную частоту operatora 4 (x: октава 8-F; y: частота)";
    strings["10xy: Setup LFO (x: enable; y: speed)##sesd"].plurals[0] = "10xy: Ustaw LFO (x: właczone; y: szybkość)";
    strings["55xy: Set SSG envelope (x: operator from 1 to 4 (0 for all ops); y: 0-7 on, 8 off)##sesd"].plurals[0] = "55xy: Ustaw obwiednię SSG (x: operator 1-4 (0 dla wszystkich operatorów); y: 0-7 вкл., 8 выкл.)";
    ...
    strings["16xy: Set operator multiplier (x: operator from 1 to 2; y: multiplier)##sesd"].plurals[0] = "16xy: Множитель частоты (x: operator 1-2; y: множитель)";
    ...
    strings["50xy: Set AM (x: operator from 1 to 2 (0 for all ops); y: AM)##sesd"].plurals[0] = "50xy: Включить АМ (x: operator 1-2 (0 dla wszystkich operatorów); y: АМ)";
    strings["51xy: Set sustain level (x: operator from 1 to 2 (0 for all ops); y: sustain)##sesd"].plurals[0] = "51xy: Уровень сустейна (x: operator 1-2 (0 dla wszystkich operatorów); y: сустейн)";
    strings["52xy: Set release (x: operator from 1 to 2 (0 for all ops); y: release)##sesd"].plurals[0] = "52xy: Релиз (x: operator 1-2 (0 dla wszystkich operatorów); y: релиз)";
    strings["53xy: Set vibrato (x: operator from 1 to 2 (0 for all ops); y: enabled)##sesd"].plurals[0] = "53xy: Вибрато (x: operator 1-2 (0 dla wszystkich operatorów); y: вкл.)";
    strings["54xy: Set envelope scale (x: operator from 1 to 2 (0 for all ops); y: scale from 0 to 3)##sesd"].plurals[0] = "54xy: Масш. огиб. (x: operator 1-2 (0 dla wszystkich operatorów); y: масш. 0-3)";
    strings["55xy: Set envelope sustain (x: operator from 1 to 2 (0 for all ops); y: enabled)##sesd"].plurals[0] = "55xy: Сустейн огибающей (x: operator 1-2 (0 dla wszystkich operatorów); y: вкл.)";
    ...
    strings["5Bxy: Set whether key will scale envelope (x: operator from 1 to 2 (0 for all ops); y: enabled)##sesd"].plurals[0] = "5Bxy: Масштаб. огиб. в зав. от ноты (x: operator 1-2 (0 dla wszystkich operatorów); y: вкл.)";
    ...
    strings["16xy: Set operator multiplier (x: operator from 1 to 4; y: multiplier)##sesd1"].plurals[0] = "16xy: Множитель частоты (x: operator 1-4; y: множитель)";
    ...
    strings["51xy: Set sustain level (x: operator from 1 to 4 (0 for all ops); y: sustain)##sesd1"].plurals[0] = "51xy: Уровень сустейна (x: operator 1-4 (0 dla wszystkich operatorów); y: сустейн)";
    strings["52xy: Set release (x: operator from 1 to 4 (0 for all ops); y: release)##sesd1"].plurals[0] = "52xy: Релиз (x: operator 1-4 (0 dla wszystkich operatorów); y: релиз)";
    ...
    strings["10xx: Set waveform (bit 0: triangle; bit 1: saw; bit 2: pulse; bit 3: noise)##sesd"].plurals[0] = "10xx: Ustaw kształt fali (bit 0: треуг.; bit 1: пила; bit 2: меандр; bit 3: шум)";
    ...
    strings["14xx: Set filter mode (bit 0: low pass; bit 1: band pass; bit 2: high pass)##sesd"].plurals[0] = "14xx: Ustaw tryb filtra (bit 0: dolno-przep.; bit 1: środkowo-przep.; bit 2: górno-przep.)";
    ...
    strings["24xy: Set output level register (x: operator from 1 to 4 (0 for all ops); y: level from 0 to 7)##sesd"].plurals[0] = "24xy: Регистр выходного уровня (громкости) (x: operator 1-4 (0 dla wszystkich operatorów); y: уровень 0-7)";
    strings["25xy: Set modulation input level (x: operator from 1 to 4 (0 for all ops); y: level from 0 to 7)##sesd"].plurals[0] = "25xy: Уровень входной модуляции (x: operator 1-4 (0 dla wszystkich operatorów); y: уровень 0-7)";
    strings["26xy: Set envelope delay (x: operator from 1 to 4 (0 for all ops); y: delay from 0 to 7)##sesd"].plurals[0] = "26xy: Задержка огибающей (x: operator 1-4 (0 dla wszystkich operatorów); y: задержка 0-7)";
    ...
    strings["2Fxy: Set fixed frequency block (x: operator from 1 to 4; y: octave from 0 to 7)##sesd"].plurals[0] = "2Fxy: Блок в режиме фиксированной частоты (x: operator 1-4; y: октава 0-7)";
    ...
    strings["5Bxy: Set whether key will scale envelope (x: operator from 1 to 4 (0 for all ops); y: enabled)##sesd1"].plurals[0] = "5Bxy: Масштаб. огиб. в зав. от ноты (x: operator 1-4 (0 dla wszystkich operatorów); y: вкл.)";
    strings["3xyy: Set fixed frequency F-num of operator 1 (x: high 2 bits from 0 to 3; y: low 8 bits of F-num)##sesd"].plurals[0] = "3xyy: Частота в режиме фиксированной частоты для operatora 1 (x: два старших бита 0-3; y: 8 младших битов частоты)";
    strings["3xyy: Set fixed frequency F-num of operator 2 (x: high 2 bits from 4 to 7; y: low 8 bits of F-num)##sesd"].plurals[0] = "3xyy: Частота в режиме фиксированной частоты для operatora 2 (x: два старших бита 4-7; y: 8 младших битов частоты)";
    strings["3xyy: Set fixed frequency F-num of operator 3 (x: high 2 bits from 8 to B; y: low 8 bits of F-num)##sesd"].plurals[0] = "3xyy: Частота в режиме фиксированной частоты для operatora 3 (x: два старших бита 8-B; y: 8 младших битов частоты)";
    strings["3xyy: Set fixed frequency F-num of operator 4 (x: high 2 bits from C to F; y: low 8 bits of F-num)##sesd"].plurals[0] = "3xyy: Частота в режиме фиксированной частоты для operatora 4 (x: два старших бита C-F; y: 8 младших битов частоты)";
    ...
    strings["1Cxx: Set echo feedback##sesd"].plurals[0] = "1Cxx: Ustaw sprzężenie zwrotne echo";
    strings["1Exx: Set dry output volume (left)##sesd"].plurals[0] = "1Exx: Głośność kanału (lewo)";
    strings["1Fxx: Set dry output volume (right)##sesd"].plurals[0] = "1Fxx: Głośność kanału (prawo)";
    ...
    strings["14xx: Set filter mode (bit 0: ring mod; bit 1: low pass; bit 2: high pass; bit 3: band pass)##sesd"].plurals[0] = "14xx: Ustaw tryb filtra (bit 0: modulacja kołowa; bit 1: dolno-przep.; bit 2: środkowo-przep.; bit 3: górno-przep.)";
